# -*- coding: utf-8 -*-
"""
Created on Fri Mar  3 15:41:17 2023
...
if P > Pb :
    rho_o = rho_ob * np.exp(co * (P - Pb))
elif P <= Pb : 
    rho_o = (62.4 * do + 0.0136 * rs * dg) / bo
# Massa Específica do Gás
rho_g = P * Mg / (z * R * T)    # T em K Mg =constante
...
#for vetores 
if P > Pb :
    a_rsb = (7.916 * 10 * -4) * (API ** 1.541)-(4.561 * 10-5) * (T ** 1.3911)   # T em F
    rsb = ((( Pb / 112.727 ) + 12.34 )*(dg ** 0.8439)*(10 * a_rsb)) ** 1.73184   # P em psia e T em F
    pb = ( (112.727 * (rsb * 0.577421) ) /( (dg ** 0.8439) * 10 * a_pb) ) - 1391.051   # T em F  
elif P <= Pb :
...
# Compressibilidade Isotérmica do Óleo p/ P >= Pb (PARTE 1)
if P >= Pb :
...
if P > Pb :
    bob = 1.0113 + (7.2046 * 10 * -5) * ( (rsb ** 0.3738) * (dg ** 0.2914/ do ** 0.6265) + 0.24626 * (T ** 0.5371)) ** 3.0936)   # T em F
    bo = bob * np.exp(-co * (P - Pb))
elif P <= Pb :
    bo = 1.0113 + (7.2046 * 10 ** -5) * ( (rs ** 0.3738) * (dg ** 0.2914/ do ** 0.6265) + 0.24626 * (T ** 0.5371)) ** 3.0936)   # T em F

# Compressibilidade Isotérmica do Óleo p/ P < Pb (PARTE 2)
if P < Pb :
...
z = a_z + ( (1 - a_z) / (np.exp(b_z))) + c_z * Ppr ** d_z   # euler ????????????????????????????????????
...
dz_dp = 1 /(( ( 0.62 - 0.23 * Tpr ) * Ppr + ( 0.066 / (Tpr  - 0.86) - 0.37 ) * Ppr ** 2 + ( 0.32 / (10 ** (9 * Tpr - 9) )) * Ppr ** 6 ) * np.exp(
     (( ( 0.62 - 0.23 * Tpr ) + (( 0.132 / (Tpr - 0.86) ) - 0.074) * Ppr ) + ( 1.92 / (10 ** (9 * Tpr - 9) ) ) * Ppr ** 5) )) + ( 10 ** ( 0.3106 - 0.49 * Tpr + 0.1824 * Tpr ** 2) * ( Ppr ** (((10 ** (0.3106 - 0.49 * Tpr + 0.1824 * Tpr ** 2) ) - 1 ) * ( 0.132 - 0.32 * np.log10 (Tpr) ) ) ) )
cpr = 1 / Ppr - 1 / z * dz_dp
